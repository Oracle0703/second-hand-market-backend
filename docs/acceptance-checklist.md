...
更新时间：2026-09-12
说明：本清单用于人工验收；账号密码必须由环境安全注入，不使用文档内固定默认密码。

## 1. 预检
- [ ] 后端可启动：`cd backend && APP_ENV=development ... go run ./cmd/server`
- [ ] 前端可启动：`cd frontend && npm run dev`
- [ ] 使用 `backend/scripts/bootstrap_admin` 创建的测试管理员可登录。
...
- [ ] 商品设为售罄后库存为 0，状态为 `SOLD`；补库存后为 `OFF_SHELF`，需显式上架。
...
- [ ] 在移动端关键操作按钮（上架/下架/设为售罄/完成订单/关闭订单）上执行一次点击回归，确认状态刷新正确。
