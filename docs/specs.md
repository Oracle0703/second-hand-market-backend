...
6. 分类采用商户自有两级分类，商家可在后台维护自己的一级/二级分类。
...
3. 分类按商户隔离，商家只能管理自己的分类；不同商户同名分类互不影响。
4. 后台提供当前商家的分类查询、新增、编辑、启停、删除能力。
5. 新商家注册时复制一份内置默认分类；历史商家通过显式 backfill 脚本初始化。

### 3.4 商品管理
1. 创建商品：标题、描述、二级分类、价格、成色、库存数量、图片。
2. 编辑商品：按状态执行字段级限制（见第 5.4 节）。
3. 状态操作：上架、下架、关闭、调整库存。
4. 商品列表：
   - 支持状态筛选、关键词搜索、时间排序、分页。
   - PC 默认表格；移动端默认卡片。
5. `stock` 规则：
   - `stock` 表示当前可用库存，创建商品时必须为大于 `0` 的整数。
   - 商品列表和详情通过“调整库存”入口支持 `INCREASE/DECREASE/MARK_SOLD`。
   - `DECREASE/MARK_SOLD` 不能扣成负数；`ON_SHELF` 减少到 `0` 时下架，`MARK_SOLD` 扣到 `0` 时转售出。
...
| --- | --- | --- | --- | --- |
| `DRAFT` | 标题、描述、分类、价格、成色、图片、库存 | 无 | 上架、调整库存、关闭 | 库存调整优先走独立入口，后端记录流水 |
| `ON_SHELF` | 描述、图片 | 分类、价格、成色、标题 | 下架、创建订单、调整库存、关闭 | 前端禁用禁止字段；后端二次校验拒绝越权字段 |
| `OFF_SHELF` | 标题、描述、分类、价格、成色、图片、库存 | 无 | 上架、调整库存、关闭 | 与 `DRAFT` 类似，允许完整调整后再上架 |
...
3. 平台级分类模板管理不纳入本期；本期只做商家自己的分类管理。
4. 管理员管理管理员不纳入本期。
