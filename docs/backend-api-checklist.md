...
4. 删除一级分类会级联软删除其全部下属二级分类；如果任一二级分类仍有商品引用，整次删除会被拒绝且不会删除任何分类。删除二级分类同样要求当前商家无商品引用。
...
- `GET /readyz`：数据库就绪探测；2 秒超时，成功 200，依赖不可用 503。仅返回通用状态，无数据库连接信息。
- `GET /healthz` 保持存活检查语义。
- 库存调整、创建/完成/关闭订单保持原 URL 与响应；关键审计失败改为整体回滚。

## 管理员安全设置

新增当前管理员账号读取与自助改密入口，详见 [管理员安全改密与浏览器回归](admin-password-security.md)。管理员通过 `/admin/security` 修改自身密码；成功后所有旧会话失效，返回 `/admin/login`。
