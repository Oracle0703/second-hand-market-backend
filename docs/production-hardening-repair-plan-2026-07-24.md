...
| A2c 管理员口令 | 保留两个管理员并逐个改密；移除固定密码 seed；不得动 `yaner` | **下一小修** | 先补改密与生产默认值保护，再逐个轮换并验证 |
| A3 备份证据 | 核实云快照 / DB 备份 / root cron；确无则补最小异机备份 | **待办** | 是生产数据库迁移前置条件 |
| C 商家多库存 | 数量、单价、自动总价、预占和多 active 订单 | **待开发** | 当前主要功能修复 |
...
| 场景 | 旧行为 | 新行为（本阶段强制） |
| --- | --- | --- |
| 创建订单成功 | 商品 → `LOCKED`，写入 `active_order_id` | 商品**保持**原 `ON_SHELF` 或 `OFF_SHELF`；不写 `active_order_id`；只增加 `reserved_stock` |
