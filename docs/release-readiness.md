...
- F-04/F-13 营业执照私有访问与管理员预览已在代码侧实现；隔离 MySQL 8.4 测试服务器审核未完成，生产未执行 `0007`、未部署且未修改现有生产文件。
- F-05 小程序 401 token refresh 实现完成；Node `19.7.0` 复演已验证修正后的 committed whitelist 包含 `miniapp/.nvmrc`，但锁定 Node `22.22.2` / npm `10.9.7` 的本地完整门禁及隔离测试服务器审核均待完成，生产小程序包未发布。
...
| --- | --- | --- | --- |
| F-01 生产弱默认值与管理员凭据 | 代码侧防护已实现：生产拒绝默认 JWT/示例数据库口令和空管理员库，固定管理员 seed 已移除，安全 bootstrap、自助改密及改密后 session 吊销已实现 | 生产数据克隆中的管理员安全矩阵通过 | 现有管理员与应用/root 数据库口令尚未在批准维护窗完成轮换；不得标记为生产关闭 |
| F-03/F-07 多库存订单 schema 与库存事务 | 代码侧已修复：`0004` 三段门禁、普通 `(product_id,is_active)` 索引、订单数量/单价/总价、库存预占、完成双减、关闭释放和多 active 订单已实现 | 生产数据克隆 MySQL 8.4.8 迁移、索引、CHECK、AutoMigrate、并发和浏览器矩阵通过 | `0004` 未执行、应用未部署，现有生产商品和订单未修改 |
...
| F-10 Git 跟踪本地业务数据库 | **未修复**：`backend/app.db` 当前仍被 Git 跟踪，且可从 4 个历史提交到达；忽略规则不能清除当前索引或历史对象 | 不适用；需要仓库对象与引用审计，不是测试服务器行为门禁 | 不修改线上数据库；历史改写、对象清理和强推均未获授权 |
...
| F-04/F-13 营业执照私有访问与管理员预览 | 代码侧已实现：公开路径仅允许商品图，管理员鉴权 Blob 预览与审计 fail-closed 已实现 | **未审核**；专用 MySQL 8.4 测试服务器审核未完成 | 未执行 `0007`、未部署、未修改现有生产文件 |
| F-05 miniapp 401 token refresh | 实现完成：单航刷新、单次重放与失败清理已实现 | Node `19.7.0` 复演已验证 `.nvmrc` 白名单完整性；锁定 Node `22.22.2` / npm `10.9.7` 的本地完整门禁及隔离测试服务器审核均待完成 | 生产小程序包未发布 |
...
| F-12 生产买家 mock 身份与迁移 | **未实现**：原三表设计/规格已批准，但“一个买家最多属于一个 PENDING 任务”缺少数据库级并发不变量；四表 claim 修订仍待批准 | 未审核；`0010`、本地门禁和专用 MySQL 8.4 harness 尚不存在 | 未迁移、未部署、未修改生产买家或 session；真实平台与生产迁移均需独立授权 |
...
- F-05：按原 Task 5 白名单构造的隔离源码因缺少 `miniapp/.nvmrc` 在工具链锁测试中产生 1 个 `ENOENT`，其余 35/36 测试通过；补入同一 `HEAD` 的 `.nvmrc` 后 focused 20/20、全量 12 files / 36 tests、微信与抖音构建均退出 0。该 Node `19.7.0` 复演仅验证白名单完整性，不替代锁定 Node `22.22.2` / npm `10.9.7` 的本地完整门禁或测试服务器审核，未启动或部署构建产物。
...
F-03/F-07、frontend Vitest、F-02、F-04/F-13、F-06、F-08 logout、F-09、F-11、F-15 与 F-16 已在本地代码侧关闭；F-01 的代码防护已实现，但现有管理员和数据库口令轮换仍是独立生产门禁。F-05 实现完成，但 Node `19.7.0` 复演仅验证了 `.nvmrc` 白名单完整性，锁定 Node `22.22.2` / npm `10.9.7` 的本地完整门禁及隔离测试服务器审核均待完成。F-02/F-09/F-11/F-16 也已通过隔离 MySQL 8.4 测试服务器审核。F-04/F-13、F-05、F-06、F-14 与 F-15 的专用测试服务器门禁仍开放；F-15 首次运行因验收 harness 所有权边界失败，修复后的第二次运行待重新授权。F-10 仍因当前跟踪和历史对象开放，F-12 的 F-11 前置已经满足但四表 claim 修订、实现和验收仍未完成。生产 frontend/backend bundle、miniapp 包与 `0004`/`0005`/`0006`/`0007`/`0008`/`0009` 仍须在各自批准的发布或维护窗部署/执行后才对线上生效，不能据此标记为生产已完成。
