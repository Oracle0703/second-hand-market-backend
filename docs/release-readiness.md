...
- 本分支依赖 P1 PR #61；合并顺序由维护者决定。
- 运行时新增 `/readyz` 与最多 30 秒优雅停机；CD 的 API 容器配置 `stop_grace_period: 35s`，覆盖应用排空时限。
- 库存/订单关键审计失败会使业务回滚，应关注审计表可写性。
- main CI 成功会触发名为 staging 的自动部署；上线前确认实际目标，不能把环境名称视作生产隔离证明。
- 小程序新增构建检查不上传平台；原有人工发布验收仍保留。
