...
3. 在 `backend/` 目录执行 MySQL 迁移：
  ...
  遇到未知列/索引、重复未关闭记录、状态不一致时停止，不自动删除或修正业务数据。
   支持 0002 的非空字段定义和早期 GORM 的可空字段/无符号 ID 定义；必须匹配完整已知布局，
   不接受任意混合。可空定义下仍要求现有意向单号、买家 ID、商品 ID 非空。
   迁移保留既有字段定义和记录，只调整本次生成列与索引。
4. SQLite staging 使用其自己的 `DB_DSN`，停止写入后执行：
  ...
  该命令只修改已有 `buyer_intents` 表的索引，DDL 放在一个事务中；不会初始化其他表或账号。
   如果 staging 数据库尚无应用表，需要先进行独立的测试环境初始化，再执行检查；
   不能把 `/readyz` 成功视为业务表已就绪，也不能导入生产数据来充当测试样本。
...
- 同一 MySQL 矩阵还覆盖实际部署沿用的早期 GORM 字段布局，验证迁移重跑、三轮关闭、
  原字段不变、只读检查通过，以及空关键字段和未知混合布局在 DDL 前被拒绝。
