...
// VerifyBuyerIntentSchema checks the deployed contract without changing data.
func VerifyBuyerIntentSchema(db *gorm.DB) error {
	return verifyBuyerIntentOpenUniqueness(db)
}

// MigrateSQLiteBuyerIntentSchema is a narrow maintenance operation for an
// existing SQLite table. Production MySQL uses the checked-in 0011 migration.
func MigrateSQLiteBuyerIntentSchema(db *gorm.DB) error {
	if db.Dialector.Name() != "sqlite" {
		return fmt.Errorf("buyer intent SQLite migration requires sqlite")
	}
	return db.Transaction(migrateSQLiteBuyerIntentOpenUniqueness)
}
...
func migrateBuyerIntentOpenUniqueness(db *gorm.DB) error {
	...
	case "sqlite":
		return MigrateSQLiteBuyerIntentSchema(db)
	case "mysql":
...
func verifyBuyerIntentRows(db *gorm.DB) error {
	...
	err := db.Table("buyer_intents").Where(fmt.Sprintf(`
		buyer_id IS NULL OR product_id IS NULL OR intent_no IS NULL OR CASE
			...
			ELSE 1
		END = 1`, statusColumn, statusColumn),
