...
func TestBuyerIntentSQLiteConstraintAllowsHistoryAndRejectsSecondOpen(t *testing.T) {
	...
	if !isIdempotencyDuplicate(err) {
		t.Fatalf("second open error = %v, want unique-key error", err)
	}
}
...
func TestBuyerIntentHistorySurvivesRepeatedDevelopmentMigrations(t *testing.T) {
	db := openBuyerIntentSchemaTestDB(t)
	if err := MigrateSchema(db); err != nil {
		t.Fatal(err)
	}
	for i, status := range []string{model.IntentClosed, model.IntentClosed, model.IntentNew} {
		intent := model.BuyerIntent{IntentNo: fmt.Sprintf("history-%d", i), BuyerID: 7, ProductID: 9, Status: status, IsOpen: status == model.IntentNew}
	...
	if after := snapshotBuyerIntentRows(t, db); !bytes.Equal(before, after) {
		t.Fatal("repeated development migration changed intent history")
	}
}
