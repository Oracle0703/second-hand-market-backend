...
func TestStockAdjustmentRollsBackIfIdempotencyResultCannotBeSaved(t *testing.T) {
	server := newIdempotencyTestServer(t)
	if err := server.DB.AutoMigrate(&model.Product{}, &model.ProductStockAdjustment{}, &model.OperationLog{}); err != nil {
		t.Fatal(err)
	}
	product := model.Product{ProductNo: "rollback-stock", MerchantID: 7, Status: model.ProductOnShelf, Stock: 5, Version: 1}
	if err := server.DB.Create(&product).Error; err != nil {
		t.Fatal(err)
	}
	if err := server.DB.Callback().Update().Before("gorm:update").Register("test:fail-stock-result", func(tx *gorm.DB) {
		if tx.Statement.Table == "idempotency_records" {
			tx.AddError(errForcedFinalize)
		}
	}); err != nil {
	...
	router := p1Router(server)
	path := fmt.Sprintf("/products/%d/stock", product.ID)
	body := `{"adjustment_type":"INCREASE","quantity":2,"reason":"stock count"}`
	result := p1Request(router, http.MethodPost, path, body, "rollback-stock-key")
	if result.Code == 0 {
		t.Fatal("expected result persistence failure")
	}
	if err := server.DB.First(&product, product.ID).Error; err != nil {
		t.Fatal(err)
	}
	if product.Stock != 5 {
		t.Fatalf("stock leaked from rolled-back transaction: %d", product.Stock)
	}
	for _, table := range []interface{}{&model.ProductStockAdjustment{}, &model.IdempotencyRecord{}, &model.OperationLog{}} {
		if countIdempotencyTestRows(t, server.DB, table) != 0 {
			t.Fatalf("rollback leaked rows in %T", table)
		}
	}
	server.DB.Callback().Update().Remove("test:fail-stock-result")
	result = p1Request(router, http.MethodPost, path, body, "rollback-stock-key")
	if result.Code != 0 {
		t.Fatalf("retry failed: %+v", result)
	}
	replay := p1Request(router, http.MethodPost, path, body, "rollback-stock-key")
	if replay.Code != 0 || replay.Data["movement_id"] != result.Data["movement_id"] {
		t.Fatalf("retry did not replay: %+v", replay)
	}
}
