...
evidence_publish_lock_identity=""
evidence_publish_final=""
evidence_publish_final_identity=""
evidence_publication_commit_started=0
...
source_export_dir_identity=""
source_export_cwd_active=0
source_export_return_dir=""
...
source_export_dir_is_current() {
  local identity=""
  [[ "$source_export_dir_owned" -eq 1 && -d "$source_export_dir" && ! -L "$source_export_dir" ]] || return 1
  identity="$(directory_identity "$source_export_dir")" || return 1
  [[ -n "$source_export_dir_identity" && "$identity" == "$source_export_dir_identity" ]]
}
...
source_export_on_exit() {
  ...
  trap - EXIT INT TERM
  set +e
  if [[ "$source_export_complete" -ne 1 && "$source_export_cwd_active" -eq 1 ]]; then
    if [[ -n "$source_export_dir_identity" &&
      "$(directory_identity . 2>/dev/null)" == "$source_export_dir_identity" ]]; then
      rm -f -- source-files.z source-sha256.txt source.tar package-sha256.txt
    fi
    cd "$source_export_return_dir" || true
    source_export_cwd_active=0
  fi
  if [[ "$source_export_complete" -ne 1 && -n "$source_export_dir" &&
    "$source_export_dir" == /* && "$source_export_dir" != / ]] && source_export_dir_is_current; then
    rmdir -- "$source_export_dir" || true
  fi
...
export_head_source() {
  ...
  for command in git sha256sum sort mktemp tar chmod mkdir rm tr cmp find xargs stat; do command -v "$command" >/dev/null || { echo "required source export command is unavailable: $command" >&2; return 1; }; done
  source_export_dir="$export_dir"
  source_export_complete=0
  source_export_dir_owned=0
  source_export_dir_identity=""
  source_export_cwd_active=0
  source_export_return_dir="$PWD"
  ...
  cd "$export_dir" || { echo "cannot enter source export destination" >&2; return 1; }
  source_export_cwd_active=1
  source_export_dir_identity="$(directory_identity .)" || { echo "cannot bind source export destination identity" >&2; return 1; }
  source_export_dir_owned=1
  mkdir "$extracted"
  chmod 700 "$export_runtime" "$extracted"
  if ! {
    chmod 700 . || exit 1
  ...
  [[ "$(directory_identity .)" == "$source_export_dir_identity" ]] || { echo "source export destination capability changed" >&2; return 1; }
  source_export_dir_is_current || { echo "source export destination identity changed" >&2; return 1; }
  cd "$source_export_return_dir" || { echo "cannot leave source export destination" >&2; return 1; }
  source_export_cwd_active=0
  rm -r -- "$export_runtime"
  source_export_runtime=""
...
prepare_evidence_parent() {
  ...
  cd "$evidence_parent" || return 1
  evidence_parent_identity="$(directory_identity .)" || { cd "$return_dir" || true; return 1; }
  cd "$return_dir" || return 1
  evidence_parent_is_current
}

evidence_child_is_current() {
  local name="$1" expected_identity="$2" identity=""
  [[ -n "$expected_identity" && -d "$name" && ! -L "$name" ]] || return 1
  identity="$(directory_identity "$name")" || return 1
  [[ "$identity" == "$expected_identity" ]]
}

restore_evidence_ambiguity_lock() {
  if [[ -n "$evidence_publish_lock" && ! -e "$evidence_publish_lock" && ! -L "$evidence_publish_lock" ]]; then
    mkdir "$evidence_publish_lock" 2>>"$runtime_dir/evidence-publish-errors.raw" || return 1
    evidence_publish_lock_identity="$(directory_identity "$evidence_publish_lock")" || return 1
  fi
}
...
publish_evidence_directory() {
  ...
  cd "$evidence_publish_lock" || {
    leave_evidence_parent_capability 1
    return 1
  }
  evidence_publish_lock_identity="$(directory_identity .)" || {
    leave_evidence_parent_capability 1
    return 1
  }
  cd .. || {
  ...
  if ! evidence_parent_is_current ||
    ! evidence_child_is_current "$evidence_publish_lock" "$evidence_publish_lock_identity" ||
    [[ -e "$evidence_name" || -L "$evidence_name" ]]; then
    leave_evidence_parent_capability 1
    return 1
  fi
  evidence_publish_final="$evidence_name"
  mkdir "$evidence_publish_final" || {
    leave_evidence_parent_capability 1
    return 1
  }
  cd "$evidence_publish_final" || {
    leave_evidence_parent_capability 1
    return 1
  }
  evidence_publish_final_identity="$(directory_identity .)" || {
    leave_evidence_parent_capability 1
    return 1
  }
  if ! chmod 700 .; then
  ...
  if ! ( ( cd "$directory" && tar -cf - . 2>>"$runtime_dir/evidence-copy-errors.raw" ) |
    tar -C . -xf - 2>>"$runtime_dir/evidence-copy-errors.raw" ); then
    leave_evidence_parent_capability 1
    return 1
  fi
  if [[ "$(directory_identity . 2>/dev/null)" != "$evidence_publish_final_identity" ]] ||
    ! validate_evidence_staging_copy "$directory" . ||
    [[ "$(directory_identity .. 2>/dev/null)" != "$evidence_parent_identity" ]] ||
    ! evidence_child_is_current "../${evidence_publish_lock#./}" "$evidence_publish_lock_identity"; then
    leave_evidence_parent_capability 1
    return 1
  fi
  cd .. || {
  ...
  if ! evidence_parent_is_current ||
    ! evidence_child_is_current "$evidence_publish_final" "$evidence_publish_final_identity" ||
    ! evidence_child_is_current "$evidence_publish_lock" "$evidence_publish_lock_identity"; then
    leave_evidence_parent_capability 1
    return 1
  fi
  evidence_publication_commit_started=1
  if ! rmdir "$evidence_publish_lock"; then
    restore_evidence_ambiguity_lock || true
    evidence_publication_commit_started=0
    leave_evidence_parent_capability 1
    return 1
  fi
  if ! evidence_parent_is_current ||
    ! evidence_child_is_current "$evidence_publish_final" "$evidence_publish_final_identity" ||
    ! validate_evidence_staging_copy "$directory" "$evidence_publish_final"; then
    restore_evidence_ambiguity_lock || true
    evidence_publication_commit_started=0
    leave_evidence_parent_capability 1
    return 1
...
on_exit() {
  ...
  if [[ "$evidence_publication_commit_started" -eq 1 && "$evidence_parent_cwd_active" -eq 1 &&
    "$(directory_identity . 2>/dev/null)" == "$evidence_parent_identity" &&
    -n "$evidence_publish_lock" && ! -e "$evidence_publish_lock" && ! -L "$evidence_publish_lock" ]]; then
    mkdir "$evidence_publish_lock" 2>>"$runtime_dir/evidence-publish-errors.raw" || true
  fi
