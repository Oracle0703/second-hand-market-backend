...
describe('miniapp authenticated request refresh', () => {
  ...
  test('does not refresh recursively when the replay is also unauthorized', async () => {
    let refreshCalls = 0
  ...
  test('does not refresh or clear another session for a skipAuth 401', async () => {
    const request = vi.fn(async () => apiResponse(401, 10002))
    const { apiRequest, useSessionStore } = await loadRequestModule(request)
    ...
    ).rejects.toThrow('request failed: status=401')
